...
class Mine_Sweeper(QMainWindow):
    def __init__(self, rows_: int, cols_: int, mines_: int) -> None:
        super().__init__()
    ...
    def set_UI(self) -> None:
        ...
        self.setFixedSize(QSize(self.cols * 40, self.rows * 40))
        self.setWindowTitle('MineSweeper QT')

        self.field: list[list[Cell]] = [
            [Cell(is_barrier=True) for _ in range(self.cols + 2)],
            *[[Cell(is_barrier=True), *[Cell() for _ in range(self.cols)],
                Cell(is_barrier=True)] for _ in range(self.rows)],
            [Cell(is_barrier=True) for _ in range(self.cols + 2)],
        ]
    ...
    def connect_signals(self) -> None:
        for i in range(1, self.rows + 1):
            for j in range(1, self.cols + 1):
                ...
                cell.run_bfs_signal.connect(self.breadth_first_search)
    ...
    def place_mines_avoiding(self, click_pos: tuple[int, int]) -> None:
        ...
        for di in (-1, 0, 1):
            for dj in (-1, 0, 1):
                ni, nj = ci + di, cj + dj
                if 1 <= ni <= self.rows and 1 <= nj <= self.cols:
                    forbidden.add((ni, nj))

        mines_set = set()
        while len(mines_set) < self.mines:
            m = (randint(1, self.rows), randint(1, self.cols))
            if m in forbidden:
                continue
            mines_set.add(m)
    ...
    def on_cell_opened(self, cell: Cell) -> None:
        ...
        self.counter += 1
        if self.counter == self.rows * self.cols - self.mines:
            ...
            for di in (-1, 0, 1):
                for dj in (-1, 0, 1):
                    if di == 0 and dj == 0:
                        continue
                    ni, nj = i + di, j + dj
                    if not (1 <= ni <= self.rows and 1 <= nj <= self.cols):
                        continue
                    if (ni, nj) in visited:
                        continue

                    neighbor = self.field[ni][nj]
                    if neighbor.is_mine or neighbor.is_barrier:
                        continue
